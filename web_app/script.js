...
async function getTotalOwed(user) {
	...
	for (let creditor of users) {
		let currentDept = await BlockchainSplitwise.lookup(user, creditor);
		debt += parseInt(currentDept);
	}
...
async function getLastActive(user) {
	time = parseInt(await BlockchainSplitwise.lastActives(user));
...
async function add_IOU(creditor, amount) {
    ...
    const user = defaultAccount.toLowerCase();

    // find path first
    let path = await doBFS(creditor.toLowerCase(), user, getNeighbors);

    let tx = await BlockchainSplitwise.connect(signer).add_IOU(creditor, amount, path ?? []);
    await tx.wait();
}
